                ...
                <div class="space-y-6">
                    ...
                    <div>
                        <label for="is_root" class="inline-flex items-center">
                            <input type="checkbox" id="is_root" name="is_root" value="1"
                                   ...
                                   {{ old('is_root', $category->is_root) ? 'checked' : '' }}>
                        ...
                        <select id="parent_id" name="parent_id"
                            ...
                            @foreach($categories as $categoryOption)
                                @if($categoryOption->id !== $category->id)
                                    <option value="{{ $categoryOption->id }}" data-root-type="{{ $categoryOption->root_type }}" {{ old('parent_id', $category->parent_id) == $categoryOption->id ? 'selected' : '' }}>
                                        {{ $categoryOption->name }}
                                    </option>
                                @endif
                            @endforeach
                    ...
                    <div id="root_type_div">
                        ...
                        <select id="root_type" name="root_type"
                            ...
                            <option value="Médicaments" {{ old('root_type', $category->root_type) == 'Médicaments' ? 'selected' : '' }}>Médicaments</option>
                            <option value="Parapharmacie" {{ old('root_type', $category->root_type) == 'Parapharmacie' ? 'selected' : '' }}>Parapharmacie</option>
                        </select>
                        @error('root_type')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        @if($category->image)
                            <div class="mb-2">
                                <img src="{{ $category->image }}" alt="{{ $category->name }}" class="w-20 h-20 object-cover rounded-lg">
